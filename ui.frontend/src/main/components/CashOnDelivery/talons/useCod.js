...
import DEFAULT_OPERATIONS from './checkcod.gql';
 ...
 * @param {DocumentNode} props.operations.getCheckcodConfigQuery query to fetch config from backend
 ...
 * }
 */
export const useCod = props => {
    const operations = mergeOperations(DEFAULT_OPERATIONS, props.operations);

    const {
        getCheckcodConfigQuery,
        setPaymentMethodOnCartMutation
    } = operations;

    const [{ cartId }] = useCartContext();
    const { data } = useQuery(getCheckcodConfigQuery);
    ...
    return {
        payableTo:
            data &&
            data.storeConfig &&
            data.storeConfig.payment_cashondelivery_payable_to,
        mailingAddress:
            data &&
            data.storeConfig &&
            data.storeConfig.payment_cashondelivery_mailing_address,
